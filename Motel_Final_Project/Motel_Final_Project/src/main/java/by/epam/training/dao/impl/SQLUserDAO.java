...
public class SQLUserDAO implements UserDAO {
	...
	private static final String UN_BAN_USER_SQL = "UPDATE users SET Is_banned='0' WHERE Login=?";
	...
	public boolean getIsBanned(String userID) throws DAOException{
		...
		try{
			...
			resultSet = statement.executeQuery();
			if( resultSet.next()){
				isBanned = resultSet.getLong("Is_banned") == 1;
			}
	...
	public boolean unBanUser(String userID) throws DAOException {
		...
		logger.info("You call unBanUser.");
		Connection connection = null;
		PreparedStatement statement = null;
		int countRows;
		try{
			...
			//create user
			statement = connection.prepareStatement(UN_BAN_USER_SQL);
			logger.info("Statement is OK.");
			statement.setString(1, String.valueOf(userID));
			countRows = statement.executeUpdate();
		...
		if( countRows > 0){
			...
			logger.info("User hsn't been unbanned.");
			logger.info("=============================================");
			return false;
		}
	}
	...
	public boolean addFunds(String userID, String toAdd) throws DAOException {
		...
		try{
			...
			connection = ConnectionPoolImpl.getInstance().takeConnection();
			if (toAdd.isEmpty() || toAdd.length() < 1){
				throw new SQLException("Incorrect toAdd parameter.");
			}
			willBeOnAccount = Long.parseLong(toAdd) +  Long.parseLong(getCashAccount(userID));
			logger.info("toAdd: " + Long.parseLong(toAdd));
			logger.info("will be on the account: " + willBeOnAccount);
			logger.info("Connection is created.");
			...
			throw new DAOException("Incorrect toAdd parameter.");
		}finally{
			SQLHelper.finalMethod(connection, statement);
		}
		if (willBeOnAccount > 0){
			logger.info("Money added successfully.");
			logger.info("=============================================");
			return true;
		} else {
			logger.info("Money hasn't been added.");
			logger.info("=============================================");
			return false;
		}
	}
